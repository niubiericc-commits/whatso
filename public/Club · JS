(function(){
  ...
  function connect(onOpen){
    ...
    try{
      ...
      lastError = '无法创建 WebSocket 连接：' + e.message;
    ...
    socket.onerror = () => {
      if(myConn!==connSeq) return;
      lastError = lastError || '连接出错（网络问题，或服务器没有响应）';
      render();
    };
  }
  ...
  try{
    ...
    const app = document.getElementById('app');
    if(app) app.innerHTML = '<div class="card"><div class="err-box">脚本执行出错：' + esc(e.message) + '</div></div>';
  }
})();
